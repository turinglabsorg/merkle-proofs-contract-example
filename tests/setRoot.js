...
async function main() {
    ...
    if (configs.owner_mnemonic !== undefined) {
        ...
        try {
            console.log('Trying setting up root...')
            ...
            console.log('Merkle tree is:', tree)
            const root = tree.getRoot().toString('hex')
            console.log('Merkle root is:', root)
            const result = await nftContract.methods.setMerkleRoot("0x" + root).send({ from: configs.owner_address })
            console.log(result);
            process.exit();
        } catch (e) {
            console.log(e)
